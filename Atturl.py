# -*- coding: utf-8 -*-
import re
def get_attatch_url(htmlcontent,url):
    '''主方法，返回attach_url'''

    att_url_list=[]

    att_url_1 = get_url(Replace_Char(htmlcontent))#干扰格式替换，获取url位置部分文本。
    for i in att_url_1:#生成附件所在url
        att_url_list.append(combin_url(get_domain(url,has_http(i)), i))
    return att_url_list

def get_couple_url(htmlcontent,url):

    att_couple=[]

    att_url_1 = get_url(Replace_Char(htmlcontent))
    for i in att_url_1:
        url_couple=[i,combin_url(get_domain(url,has_http(i)), i)]
        att_couple.append(url_couple)
    return att_couple

#干扰格式替换
def Replace_Char(htmlcon):
...
#根据规则合并url。
def combin_url(domain,att_url):

    if att_url=='':
        return att_url

    elif has_http(att_url)==0:
        return att_url
    elif has_http(att_url)>=1:
...
def check():
    att_url_1 =Replace_Char(htmlcontent)
